<template>
	<el-input v-model="filterVal" :prefix-icon="Search" :placeholder="t('message.pages.menu.form.menuNamePlaceholder')" />
...
const emit = defineEmits(['treeClick', 'deleteDept', 'updateDept', 'sortDone']);
...
/**
 * click left edit button
 */
const handleUpdateMenu = (type: string) => {
	if (type === 'update') {
		if (!treeSelectMenu.value.id) {
			warningNotification(t('message.pages.menu.tree.selectMenuWarning'));
			return;
		}
...
const handleSort = async (type: string) => {
	...
	const isRoot = treeSelectNode.value?.level === 1;
	const parentNode = isRoot ? null : treeSelectNode.value?.parent;
	...
	if (type === 'up') {
		...
		try {
			...
			emit('sortDone', { parentNode, isRoot });
		} finally {
			sortDisable.value = false;
		}
	}
	if (type === 'down') {
		if (index >= parentList.length - 1) return;
		sortDisable.value = true;
		try {
