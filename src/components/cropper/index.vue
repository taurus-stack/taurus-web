<template>
	<div>
		<el-dialog :title="t('message.importExcel.avatarCropper')" v-model="state.isShowDialog" width="769px">
			<div class="cropper-warp">
				<div class="cropper-warp-left">
					<img :src="state.cropperImg" class="cropper-warp-left-img" />
				</div>
				<div class="cropper-warp-right">
					<div class="cropper-warp-right-title">{{ t('message.preview') }}</div>
			...
			<template #footer>
				<span class="dialog-footer">
					<el-button @click="onCancel" size="default">{{ t('message.cancelBtn') }}</el-button>
					<el-button type="primary" @click="onSubmit" size="default">{{ t('message.switch') }}</el-button>
				</span>
			</template>
		</el-dialog>
	</div>
