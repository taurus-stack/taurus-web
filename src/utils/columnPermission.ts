...
/**
 * Process field permission info
 * @param func API function to fetch field info
 * @param crudOptions Original crudOptions
	...
	for (let col in columns) {
		...
		for (let item of res.data) {
			if (excludeColumns.includes(item.field_name)) {
				...
				// If the column is not visible in the list, disable it in column settings
